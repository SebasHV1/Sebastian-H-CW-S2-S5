...
<body>
    <?php
        ...
        $id= explode('=', $url);
        $peticion="SELECT NombreDClase
                FROM clases
                WHERE ID_clase='$id[1]'";
        $query=mysqli_query($conexion,$peticion);
            if($query){
                while($row=$query->fetch_array()){
                    $clase = $row['NombreDClase'];
                }
            }
        echo "
            <nav class='navbar bg-light'>
                <div class='container-fluid'>
                    ...
                    <span class='navbar-brand mb-0 h1'>$clase</span>
        ...
        if ($_SESSION["Usuario"]=='Profesor'){
            ...
            if ($_SESSION["Usuario"]=='Alumno'){
                echo"
                <button id='agregarTarea' style='display: none;'>Agregar una tarea</button>
                <button id='aceptar' class='btn-asignar' style='display: none;'>Enviar</button>
                <button id='cancelar' class='btn-asignar' style='display: none;'>Cancelar</button>  
                <br/><button id='aceptar2' style='display: none; class='btn-asignar'>Enviar</button>
                <div id='CREARCLASE' style='display: block'>
        ...
        if($_SESSION["Usuario"]=='Profesor'){
            echo "
                <button id='agregarTarea'>Agregar una tarea</button>
                <div id='contenedor-asignar' style='display: none;'>
                    <form id='asignar' enctype='multipart/form-data'>
                        <!--AQUÍ SE PONDRAN LAS TAREAS-->  
                        <label for='NomTarea'>Nombre de la asignación:</label>
                        <br/><input type='text' id='NomTarea' name='NomTarea' class='asignacionSubir' require>
                        <br/><label for='describe'>Describe la asigancion:</label>
                        <br/><input type='text' id='describe' name='describe' class='asignacionSubir' require>
                        <br/><label for='Apoyo'>Material de apoyo(opcional)</label>
                        <br/><input type='file' id='Apoyo' name='Apoyo' class='asignacionSubir'>
                        <br/><label for='FechaEntrega'>Fecha de entrega:</label>
                        <br/><input type='date' id='FechaEntrega' name='FechaEntrega' class='asignacionSubir' require>
                        <input type='number' id='idPag' value= '$id[1]' name='idPag' style='display: none;'>
                        <br/><button id='aceptar2' class='btn-asignar'>Enviar</button>
                        <button id='cancelar' class='btn-asignar'>Cancelar</button>  
                    </form>
                </div>
            ";
        ...
        echo "
            <div id='tareasPuestas'> 
        ";
                ////////CARGAR LAS TAREAS/////////
                $peticion="SELECT CHT, NombreTarea, Indicaciones_De_La_Tarea, Ruta_Archivo, Fecha_De_Entrega
                ...
                if($query){
                    while($row=$query->fetch_array()){
                        $CHT = $row['CHT'];
                        $Tarea=$row['NombreTarea'];
                        $Descripcion=$row['Indicaciones_De_La_Tarea'];
                        $Ruta = $row['Ruta_Archivo'];
                        $Fecha = $row['Fecha_De_Entrega'];
                        if($Ruta==''){
                           echo"
                                <div class='anuncio'>$Tarea<br/>Descripción: $Descripcion<br/>Fecha de entrega: $Fecha </div>
                            "; 
                        }
                        else{
                            $ext= explode(".", $Ruta);
                            $nombreArch=$Tarea.'.'.$ext[5];
                            echo"
                                <div class='anuncio'>$Tarea<br/>Descripción: $Descripcion<br/><a download='$nombreArch' href='$Ruta'>Material</a><br/>Fecha de entrega: $Fecha<br/></div>
                            "; 
                        }
            ...
            <!--para profesores-->
            <!-- <div class="anuncio">Soy una anuncio de la tarea</div> -->
        </div>
        <div id='importantes'>Importantes
            <div class='importante'>Hola soy algo importante</div>
